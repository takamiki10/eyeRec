...
from train_eye_shape.datasets import EyeShapeFolderDataset, build_eval_transforms
from train_eye_shape.model import load_model_for_inference
from train_eye_shape.utils import get_device, invert_mapping, load_label_mapping, load_yaml, resolve_path, save_json
...
def evaluate(config_path: str | Path) -> dict:
    ...
    test_dataset = EyeShapeFolderDataset(
    ...
    model_path = resolve_path(config["export_dir"], base_dir) / "eye_shape_model.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the exported eye shape classifier.")
