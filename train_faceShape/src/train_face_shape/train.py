...
from train_face_shape.datasets import FaceShapeFolderDataset, build_eval_transforms, build_train_transforms
from train_face_shape.model import build_model
from train_face_shape.utils import (
    get_device,
    invert_mapping,
    load_label_mapping,
    load_yaml,
...
def build_limited_balanced_sampler(
    ...
    counts = class_counts_from_samples(samples, num_classes)
    sample_weights = [1.0 / max(counts[label_index], 1) for _, label_index in samples]
    return WeightedRandomSampler(sample_weights, num_samples=max_samples, replacement=True)
...
def train(config_path: str | Path) -> dict:
    ...
    val_sampler = build_limited_balanced_sampler(
        val_dataset.samples,
        len(label_to_index),
        int(config.get("max_val_samples", 0)),
    )
    ...
    for epoch in range(1, int(config["epochs"]) + 1):
        ...
        if val_accuracy >= best_val_accuracy:
            ...
            epochs_without_improvement = 0
            torch.save(checkpoint, export_dir / "face_shape_model.pt")
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the local EyeRec face shape classifier.")
